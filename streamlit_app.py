...
def push_excel_to_github():
    """
    Reads hostel_data.xlsx and commits it back to GitHub.
    Credentials are read from Streamlit secrets (never hardcoded).
    Silently skips if secrets are not configured (e.g. local dev).
    """
    try:
        token  = st.secrets["github"]["token"]
        repo   = st.secrets["github"]["repo"]       # e.g. "username/hostel-allocation"
        branch = st.secrets["github"].get("branch", "main")
        path   = st.secrets["github"].get("file_path", "hostel_data.xlsx")
    except (KeyError, FileNotFoundError):
        # Running locally without secrets — skip GitHub push silently
        return

    # Read the current file SHA from GitHub (needed to update an existing file)
    api_url  = f"https://api.github.com/repos/{repo}/contents/{path}"
    headers  = {"Authorization": f"token {token}", "Accept": "application/vnd.github.v3+json"}

    sha_resp = requests.get(api_url, headers=headers, params={"ref": branch})
    if sha_resp.status_code != 200:
        st.warning(f"⚠️ Could not fetch file SHA from GitHub: {sha_resp.status_code}. Changes saved locally but not synced.")
        return
    sha = sha_resp.json()["sha"]

    # Encode the updated Excel file as base64
    ...
    if put_resp.status_code not in (200, 201):
        st.warning(f"⚠️ GitHub sync failed ({put_resp.status_code}). Changes saved locally but not pushed.")


def pull_excel_from_github():
    """
    Downloads the latest hostel_data.xlsx from GitHub into the app's working
    directory. Called once at startup so the app always starts with fresh data.
